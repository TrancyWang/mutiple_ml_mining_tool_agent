...
class XGBoostRegressionTool:
    ...
    def run_multiple_training(self):
        ...
        for i in range(self.multiple_folder):
            ...
            model = xgb.XGBRegressor(
                ...
                # ===== 修复 base_score 兼容性问题 =====
                base_score=0.5,  # 显式设置为标量值，避免数组格式
                # ===== 工程参数 =====
                random_state=42 + i,
                n_jobs=-1,
                verbosity=0
            ...
            try:
                ...
                if base_score_str.startswith('[') and base_score_str.endswith(']'):
                    ...
                    config_dict['learner']['learner_model_param']['base_score'] = str(base_score_value)
                    
                    # 将修改后的配置加载回 booster
                    new_config_str = json.dumps(config_dict)
                    booster.load_config(new_config_str)
            except Exception as e:
                print(f"Warning: Failed to fix base_score format: {e}")
    ...
    def save_shap_importance(self, output_file=None, save_dir=None):
        ...
        output_file : str, optional
            输出文件名，默认为 xgboost_shap_importance_mean.xlsx
        save_dir : str, optional
            SHAP 文件保存目录，默认为实例初始化时设置的 shap_save_dir
        """
        if output_file is None:
            output_file = "xgboost_shap_importance_mean.xlsx"
    ...
    def plot_actual_vs_predicted(self, output_file=None, save_dir=None):
        ...
        -----------
        output_file : str, optional
            输出文件名，默认为 xgb_figure_actual_vs_predicted.png
        save_dir : str, optional
            图片保存目录，默认为实例初始化时设置的 save_dir
        """
        if output_file is None:
            output_file = "xgb_figure_actual_vs_predicted.png"
    ...
    def plot_residual_analysis(self, output_file=None, save_dir=None):
        ...
        -----------
        output_file : str, optional
            输出文件名，默认为 xgb_figure_residual_plot.png
        save_dir : str, optional
            图片保存目录，默认为实例初始化时设置的 save_dir
        """
        if output_file is None:
            output_file = "xgb_figure_residual_plot.png"
    ...
    def plot_shap_summary(self, output_file=None, max_display=12, save_dir=None):
        ...
        -----------
        output_file : str, optional
            输出文件名，默认为 xgb_figure_shap_summary.png
        ...
        """
        if output_file is None:
            output_file = "xgb_figure_shap_summary.png"
        ...
        # 修复 XGBoost base_score 格式兼容性问题（如果尚未修复）
        try:
